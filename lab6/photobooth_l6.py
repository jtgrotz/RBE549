...
harris_image = False
SIFT_image = False
panorama_image = False
pan_image_count = 0
pan_images = [] #initial structure for holding panorama images
pic_number = 0
vid_number = 0
...
while vid.isOpened():
    ...
    #waits for 3 images to be collected and then performs stitching.
    if panorama_image:
        if pan_image_count > 2:
            #resets count to allow for next panorama
            pan_image_count = 0
            #calls outside stitch function
            stitched_image  = panorama.stitch_image(pan_images[0],pan_images[1],pan_images[2], sift)
            cv.imshow('Panorama', stitched_image)
            cv.waitKey(0)
            pan_images = []
    #dont do sift or harris corners if in panorama mode
    else:
        if SIFT_image:
            #convert image to grayscale
            gray_image = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
            #detect features
            feat = sift.detect(gray_image, None)
            #add features to the image
            frame = cv.drawKeypoints(frame, feat, frame, color=[255, 0, 0])

        if harris_image:
            #convert image to gray and f32
            gray_image = cv.cvtColor(frame,cv.COLOR_BGR2GRAY)
            grayf32 = np.float32(gray_image)
            #detect corners
            corners = cv.cornerHarris(grayf32, 5, 5, 0.1)
            #display corners
    ...
    #press f to enable SIFT Features
    elif k == ord('f'):
        if SIFT_image:
            SIFT_image = False
        else:
